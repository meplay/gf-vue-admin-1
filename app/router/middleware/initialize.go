...
import (
	...
	service "github.com/flipped-aurora/gf-vue-admin/app/service/system"
...
func Initialize() {
	var err error
	GfJWTMiddleware, err = jwt.New(&jwt.GfJWTMiddleware{
	...
	if err != nil {
		zap.L().Error("jwt 中间件初始化失败!", zap.Error(err))
	}
}
...
func Authenticator(r *ghttp.Request) (interface{}, error) {
	...
	if !service.Store.Verify(info.CaptchaId, info.Captcha, true) { // 验证码校对
		return nil, _errors.New("验证码错误! ")
	}
	if data, err := service.User.Login(&info); err != nil {
		_ = r.Response.WriteJson(&response.Response{Code: 7, Error: err, Err: err.Error()})
		r.ExitAll()
		return nil, nil
	} else {
		r.SetParam("user", data) // 设置参数保存到请求中
		return g.Map{"uuid": data.Uuid, "id": data.ID, "nickname": data.Nickname, "authority_id": data.AuthorityId}, nil
	}
}
