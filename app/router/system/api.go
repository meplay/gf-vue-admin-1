...
func (a *_api) Init() {
	group := a.router.Group("/api")
	{
		...
		group.POST("getApiById", a.response.Handler()(system.Api.First))          // 获取单条Api消息
		...
		group.DELETE("deleteApisByIds", a.response.Handler()(system.Api.Deletes)) // 删除选中Api
		group.POST("getApiList", a.response.Handler()(system.Api.GetList))        // 获取Api列表
