...
// arModel is a active record design model for table simple_upload  operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of simple_upload.
	Table = "simple_upload"
	// Model is the model object of simple_upload.
	Model = &arModel{g.DB(global.Db).Table(Table).Safe()}
	// Columns defines and stores column names for table simple_upload.
	Columns = struct {
		ChunkNumber      string // 当前切片标记
		CurrentChunkPath string // 切片本地路径
		CurrentChunkSize string // 当前切片容量
		FilePath         string // 文件本地路径
		Filename         string // 文件名
		Identifier       string // 文件标识（md5）
		IsDone           string // 是否上传完成
		TotalChunks      string // 切片总数
		TotalSize        string // 总容量
	}{
		ChunkNumber:      "chunk_number",
		CurrentChunkPath: "current_chunk_path",
